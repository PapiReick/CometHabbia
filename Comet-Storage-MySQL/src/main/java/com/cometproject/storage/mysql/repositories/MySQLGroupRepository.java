...
public class MySQLGroupRepository extends MySQLRepository implements IGroupRepository {
    ...
    public void getDataById(int groupId, Consumer<IGroupData> consumer) {
        select("SELECT g.id, g.category, g.name, g.description, g.tag, g.badge, g.owner_id, g.room_id, g.created, g.`type`, g.colour1, " +
                "g.colour2, g.members_deco, g.has_forum, g.currency, p.username as owner_name, p.figure as owner_figure, p.motto as owner_motto FROM groups g " +
    ...
    public void saveGroupData(IGroupData groupData) {
        update("UPDATE groups SET name = ?, description = ?, category = ?, badge = ?, owner_id = ?, room_id = ?, type = ?, " +
                        "colour1 = ?, colour2 = ?, members_deco = ?, has_forum = ?, currency = ?, tag = ? WHERE id = ?",
                groupData.getTitle(), groupData.getDescription(), groupData.getCategory(), groupData.getBadge(), groupData.getOwnerId(),
                ...
                groupData.hasForum() ? "1" : "0",
                groupData.getCurrency(), groupData.getTag(), groupData.getId());
    }

    @Override
    public void create(IGroupData groupData) {
        insert("INSERT into groups (`name`, `category`, `description`, `tag`, `badge`, `owner_id`, `room_id`, `created`, `type`, `colour1`, `colour2`, `members_deco`, `has_forum`) " +
                "VALUES(? ,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '0');", (key) -> {
            final int groupId = key.readInteger(1);

            groupData.setId(groupId);
        }, groupData.getTitle(), groupData.getCategory(), groupData.getDescription(), groupData.getTag(), groupData.getBadge(), groupData.getOwnerId(), groupData.getRoomId(),
    ...
    private IGroupData readGroup(IResultReader data) throws Exception {
        ...
        final int category = data.readInteger("category");
        ...
        final int currency = data.readInteger("currency");
        final String tag = data.readString("tag");
        ...
        return this.groupDataFactory.create(groupId, category, name, description, tag, badge, ownerId, username, roomId, created, groupType,
                colour1, colour2, membersDeco, hasForum, currency, new PlayerAvatarData(ownerId, username, ownerFigure, ownerMotto, "M"));
    }
}
