...
public class PlayerInfoCommand extends ChatCommand {
    ...
    public void execute(Session client, String[] params) {
        ...
        if (client.getPlayer().getPermissions().getRank().modTool()) {
            ...
            userInfo.append("<b>" + Locale.getOrDefault("command.playerinfo.royaleXP", "XP Royale:") + "</b>: " + (session == null ? "Disconnected" : session.getPlayer().getSettings().getRoyaleXP()) + "<br><br>");
        }

        userInfo.append("<b>" + Locale.getOrDefault("command.playerinfo.currencyBalances", "Monedas del usuario") + "</b><br>");
        userInfo.append("<i>" + playerData.getCredits() + " <b><font color='#E79D1C'>" + Locale.getOrDefault("command.playerinfo.koins", "Koins") + "</font></b></i><br>");

        if (client.getPlayer().getPermissions().getRank().modTool()) {
            userInfo.append("<i>" + playerData.getVipPoints() + " <b><font color='#F71053'>" + Locale.getOrDefault("command.playerinfo.rubys", "Rubíes") + "</font></b></i><br>");
        }

        userInfo.append("<i>" + playerData.getActivityPoints() + " <b><font color='#28BCD4'>" + Locale.getOrDefault("command.playerinfo.activityPoints", "Píxeles") + "</font></b></i><br>");
        userInfo.append("<i>" + playerData.getBlackMoney() + " <b><font color='#15C34C'>" + Locale.getOrDefault("command.playerinfo.tokens", "Tokens") + "</font></b></i><br>");
