...
public class SexCommand extends ChatCommand {
    ...
    public void execute(Session client, String[] params) {
        if (params.length != 1) {
            sendNotif(Locale.getOrDefault("command.sex.none", "Who do you want to sex?"), client);
            return;
        }
        ...
        if (punchedSession.getPlayer().getData().getUsername().equals(client.getPlayer().getData().getUsername())) {
            sendNotif(Locale.getOrDefault("command.sex.himself", "You can't fuck yourself!"), client);
            return;
        }
