...
public class KillCommand extends ChatCommand {
    ...
    public void execute(Session client, String[] params) {
        if (params.length != 1) {
            sendNotif(Locale.getOrDefault("command.kill.none", "Who do you want to kill?"), client);
            return;
        }
        ...
        if (punchedSession.getPlayer().getData().getUsername().equals(client.getPlayer().getData().getUsername())) {
            sendNotif(Locale.getOrDefault("command.kill.himself", "You can't kill yourself!"), client);
            return;
        }
        ...
        if (!((Math.abs((posX - playerX)) >= 2) || (Math.abs(posY - playerY) >= 2))) {
            client.getPlayer().getEntity().getRoom().getEntities().broadcastMessage(new WhisperMessageComposer(client.getPlayer().getEntity().getId(), "* " + client.getPlayer().getData().getUsername() + " " + Locale.getOrDefault("command.sex.word", "Starts fucking with") + " " + punchedSession.getPlayer().getData().getUsername() + " *", 34));
        } else {
